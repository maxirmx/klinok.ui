...
async function createDrugRecord() {
  ...
  try {
    const record = await saveDrugDraft();
    showToast("Препарат создан");
    router.push(`/owner/materials/drugs/${record.id}`);
  } catch (error) {
    showToast(error instanceof Error ? error.message : "Не удалось создать препарат");
  }
}

async function updateDrugRecord() {
  if (!drugRecord.value || showValidationError()) return;

  try {
    const record = await updateDrugDraft(drugRecord.value);
    showToast("Препарат сохранен");
    router.push(`/owner/materials/drugs/${record.id}`);
  } catch (error) {
    showToast(error instanceof Error ? error.message : "Не удалось сохранить препарат");
  }
}

function submitDrugRecord() {
  if (isDrugEdit.value) {
    void updateDrugRecord();
    return;
  }
  void createDrugRecord();
}
    ...
    <section class="panel materials-panel">
      ...
      <div v-else class="accordion" data-test="materials-templates-list">
        <button
          ...
          :key="template"
          class="material-row"
          data-test="mock-template-row"
          @click="showToast('Шаблон скопирован')"
        >
