...
export type DappEventType = "drug.record.saved" | "drug.record.deleted";

export interface DappEventBase {
  id: string;
  actorId: string;
  createdAt: string;
  type: DappEventType;
...
export type DappEvent =
  | (DappEventBase & { type: "drug.record.saved"; payload: { record: DrugRecord } })
  | (DappEventBase & { type: "drug.record.deleted"; payload: { id: string } });

export type DappEventInput =
  | {
      type: "drug.record.saved";
      payload: { record: DrugRecord };
      actorId?: string;
      createdAt?: string;
      id?: string;
    }
...
export type DappWatchCallback = (collections: DappCollections) => void;
